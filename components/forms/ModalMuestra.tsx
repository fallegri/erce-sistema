...
export default function ModalMuestra({ tiposMuestra, tiposEstudio, onSave, onClose, defaults }: Props) {
  const [form, setForm] = useState<MuestraForm>({
  ...
  // Use useCallback + stable field setter to prevent re-renders killing focus
  ...
  const analizarConIA = async () => {
    ...
    try {
      ...
      const tiposEstudioNombres = tiposEstudio.filter((t) => form.tipos_estudio_ids.includes(t.id)).map((t) => t.nombre)
  ...
  const handleSave = () => {
    onSave({ ...form, analisis_ia: analisis ? JSON.stringify(analisis) : null })
  }

  const formValido = form.persona_recolecto && form.fecha_recoleccion && form.pertenece_a && form.nombre_muestra && form.tipo_muestra_id
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-modal w-full max-w-2xl max-h-[90vh] flex flex-col animate-fadeIn">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              {defaults?.persona_recolecto && (
                <span className="text-brand-600">Datos heredados del registro anterior</span>
              )}
              {!defaults?.persona_recolecto && 'Completa los datos de la muestra pericial'}
            </p>
          </div>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setField('pertenece_a', e.target.value)}
              />
            </div>
            <div>
              ...
              <input
                ...
                onChange={(e) => setField('persona_recolecto', e.target.value)}
              />
            </div>
          </div>
          ...
          <div className="border border-slate-200 rounded-xl p-4 bg-slate-50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">¿Esta muestra requiere un análisis pericial especializado?</p>
              </div>
              <button
                type="button"
                onClick={() => setForm((prev) => ({ ...prev, estudio_pericial_solicitado: !prev.estudio_pericial_solicitado, tipos_estudio_ids: [], codigo_idif_manual: '' }))}
                className={clsx('relative inline-flex h-6 w-11 items-center rounded-full transition-colors duration-200', form.estudio_pericial_solicitado ? 'bg-brand-600' : 'bg-slate-300')}
              >
                <span className={clsx('inline-block h-4 w-4 rounded-full bg-white shadow transition-transform duration-200', form.estudio_pericial_solicitado ? 'translate-x-6' : 'translate-x-1')} />
              </button>
            </div>
              ...
              <div className="mt-4 space-y-4 animate-fadeIn">
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {tiposEstudioActivos.map((te) => (
                      <label key={te.id} className={clsx('flex items-center gap-2.5 px-3 py-2 rounded-lg border cursor-pointer transition-all text-sm', form.tipos_estudio_ids.includes(te.id) ? 'border-brand-500 bg-brand-50 text-brand-800' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-300')}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setField('codigo_idif_manual', e.target.value)}
                  />
                </div>
              </div>
            )}
          ...
          <div className="border border-slate-200 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-brand-50 to-slate-50 border-b border-slate-200">
              ...
              <button type="button" onClick={analizarConIA} disabled={!formValido || analizando} className="btn-secondary text-xs py-1.5 px-3 disabled:opacity-50">
                {analizando ? <span className="flex items-center gap-1.5"><svg className="animate-spin h-3 w-3" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Analizando...</span> : 'Analizar muestra'}
              </button>
            </div>
            <div className="p-4">
              {!analisis && !analizando && (
                <p className="text-xs text-slate-400 text-center py-2">
                  {formValido ? 'Presiona "Analizar muestra" para obtener recomendaciones de Gemini.' : 'Completa los campos obligatorios para habilitar el análisis.'}
                </p>
              )}
              {analizando && <div className="flex items-center justify-center gap-2 py-4 text-sm text-slate-500"><svg className="animate-spin h-4 w-4 text-brand-500" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Consultando Gemini...</div>}
              {analisis && (
                <div className="space-y-3 animate-fadeIn">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-600">Nivel de prioridad</span>
                    <span className={clsx('badge text-xs font-semibold border', priorityColor[analisis.nivel_prioridad])}>{analisis.nivel_prioridad}</span>
                  </div>
                  ...
                  {analisis.alertas.length > 0 && (
                    <div>
                      ...
                      <ul className="space-y-1">{analisis.alertas.map((a, i) => <li key={i} className="text-xs text-orange-800 bg-orange-50 rounded-lg px-3 py-1.5">{a}</li>)}</ul>
                    </div>
                  )}
                  {analisis.recomendaciones.length > 0 && (
                    <div>
                      <p className="text-xs font-medium text-blue-700 mb-1.5">✓ Recomendaciones</p>
                      <ul className="space-y-1">{analisis.recomendaciones.map((r, i) => <li key={i} className="text-xs text-blue-800 bg-blue-50 rounded-lg px-3 py-1.5">{r}</li>)}</ul>
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-100">
          ...
          <button onClick={handleSave} disabled={!formValido} className="btn-primary">Agregar muestra</button>
        </div>
      </div>
    </div>
  )
