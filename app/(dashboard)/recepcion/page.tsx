...
import { useRecepcionStore } from '@/lib/store'
...
export default function RecepcionPage() {
  ...
  // BDD Feature 2: Validación de fechas
  const fechaInvalida =
    form.fecha_roma && form.fecha_erce
      ? new Date(form.fecha_erce) < new Date(form.fecha_roma)
      : false

  const formularioValido =
    form.funcionario_entrega &&
    form.fecha_roma &&
    form.fecha_erce &&
    !fechaInvalida &&
    muestras.length > 0
  ...
  useEffect(() => {
    fetch('/api/parametros')
      ...
      .then((d) => {
        if (d.ok) {
          setTiposMuestra(d.data.tiposMuestra)
          setTiposEstudio(d.data.tiposEstudio)
        }
      })
  }, [])
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      if (!data.ok) {
        ...
        // BDD Feature 4: Generar QR por cada muestra
        const qrData: MuestraQRData[] = data.data.muestras.map((m: MuestraQRData) => ({
          ...m,
          fecha_erce: form.fecha_erce,
          funcionario_entrega: form.funcionario_entrega,
        }))
        setMuestrasQR(qrData)
        setShowQR(true)
        reset()
      }
    } catch {
  ...
  const getTipoNombre = (id: number | null) =>
    tiposMuestra.find((t) => t.id === id)?.nombre ?? '—'
          ...
          <div className="card p-6 space-y-5 sticky top-0">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setForm({ funcionario_entrega: e.target.value })}
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Fecha ROMA (recolección) *
              </label>
              <input
                type="date"
                className="input-base"
                value={form.fecha_roma}
                onChange={(e) => setForm({ fecha_roma: e.target.value })}
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">
                Fecha ERCE (recepción) *
              </label>
              <input
                type="date"
                className={clsx('input-base', fechaInvalida && 'border-red-400 focus:border-red-500 focus:ring-red-100')}
                value={form.fecha_erce}
                onChange={(e) => setForm({ fecha_erce: e.target.value })}
              />
              {/* BDD Feature 2: Alerta roja de fecha */}
              {fechaInvalida && (
                <div className="flex items-center gap-1.5 mt-1.5">
                  ...
                  <p className="text-xs text-red-600 font-medium">
                    La fecha ERCE no puede ser anterior a la fecha ROMA.
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-between py-2 px-3 bg-slate-50 rounded-lg">
              <div>
                ...
                <p className="text-xs text-slate-500">Estado actual del expediente</p>
              </div>
              <button
                type="button"
                onClick={() => setForm({ caso_abierto: !form.caso_abierto })}
                className={clsx(
                  'relative inline-flex h-6 w-11 items-center rounded-full transition-colors',
                  form.caso_abierto ? 'bg-brand-600' : 'bg-slate-300'
                )}
              >
                <span className={clsx(
                  'inline-block h-4 w-4 rounded-full bg-white shadow transition-transform',
                  form.caso_abierto ? 'translate-x-6' : 'translate-x-1'
                )} />
              </button>
            </div>
            ...
            {muestras.length === 0 && (
              <p className="text-xs text-slate-400 text-center">
                Agrega al menos una muestra para poder guardar.
              </p>
            )}
          </div>
        </div>

        {/* ── DETALLE: Lista de muestras ── */}
            ...
            <div className="space-y-3">
              {muestras.map((m, idx) => (
                <div key={m.tempId} className="card p-4 flex gap-4 animate-slideIn hover:border-slate-300 transition-colors">
                  <div className="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center shrink-0 font-display font-bold text-brand-700 text-sm">
                    {idx + 1}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <p className="text-xs text-slate-500 mt-0.5">
                          {getTipoNombre(m.tipo_muestra_id)} · {m.pertenece_a}
                        </p>
                      </div>
                      <button
                        onClick={() => removeMuestra(m.tempId)}
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      <span className="badge-info">
                        Recol: {m.fecha_recoleccion}
                      </span>
                      {m.estudio_pericial_solicitado && (
                        <span className="badge-warning">Estudio pericial</span>
                      )}
                      {m.codigo_idif_manual && (
                        <span className="badge-gray font-mono">IDIF: {m.codigo_idif_manual}</span>
                      )}
                      {(m as any).analisis_ia && (
                        <span className="badge-success">IA analizada</span>
                      )}
                    </div>
                  </div>
                </div>
