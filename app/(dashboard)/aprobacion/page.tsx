  ...
  const ejecutarAccion = async (id: number, tipo: string) => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        showToast(data.error || 'Error al ejecutar acción.', false)
      }
    } catch {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-modal w-full max-w-sm p-6 animate-fadeIn">
            <div className={clsx('w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-4',
              ...
              {confirmacion.tipo === 'activar' && <svg className="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>}
              {confirmacion.tipo === 'hacer_admin' && <svg className="w-6 h-6 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>}
            </div>
            ...
            <p className="text-sm text-slate-500 text-center mb-5">
              {confirmacion.tipo === 'aprobar' && <>¿Confirmas la aprobación de <strong className="text-slate-800">{confirmacion.nombre}</strong>? Recibirá acceso al sistema con rol ERCE.</>}
              {confirmacion.tipo === 'bloquear' && <>¿Deseas bloquear a <strong className="text-slate-800">{confirmacion.nombre}</strong>? No podrá iniciar sesión hasta ser reactivado.</>}
              {confirmacion.tipo === 'activar' && <>¿Deseas reactivar a <strong className="text-slate-800">{confirmacion.nombre}</strong>? Recuperará acceso al sistema.</>}
              {confirmacion.tipo === 'hacer_admin' && <>¿Deseas promover a <strong className="text-slate-800">{confirmacion.nombre}</strong> como administrador? Tendrá acceso completo al sistema.</>}
            </p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmacion(null)} className="btn-secondary flex-1 justify-center">Cancelar</button>
              <button
                onClick={() => ejecutarAccion(confirmacion.id, confirmacion.tipo)}
                disabled={accionando === confirmacion.id}
      ...
      <div className="flex items-start justify-between">
        <div>
          <h1 className="font-display text-2xl font-bold text-slate-900">Aprobación de usuarios</h1>
          <p className="text-sm text-slate-500 mt-0.5">Gestión de solicitudes de acceso y control de cuentas</p>
        </div>
        {pendientes.length > 0 && (
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={label} className={clsx('card p-4 flex items-center gap-4',
            color === 'amber' && count > 0 ? 'border-amber-200 bg-amber-50' : '')}>
            <div className={clsx('w-10 h-10 rounded-xl flex items-center justify-center text-lg shrink-0',
              color === 'amber' ? 'bg-amber-100' : color === 'green' ? 'bg-green-100' : 'bg-red-100')}>
              {icon}
            </div>
            <div>
        ...
        <div className="flex rounded-lg border border-slate-200 overflow-hidden">
          <button onClick={() => setVista('pendientes')}
            className={clsx('px-4 py-2 text-sm font-medium transition-colors',
              vista === 'pendientes' ? 'bg-brand-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50')}>
            Solo pendientes
          </button>
          <button onClick={() => setVista('todos')}
            className={clsx('px-4 py-2 text-sm font-medium transition-colors border-l border-slate-200',
              vista === 'todos' ? 'bg-brand-600 text-white' : 'bg-white text-slate-600 hover:bg-slate-50')}>
            Todos los usuarios
          </button>
        </div>
        <input
          className="input-base text-sm flex-1 max-w-sm"
          placeholder="Buscar por nombre, email, CI o ciudad..."
          value={busqueda}
      ...
      {/* Lista de usuarios */}
      {loading ? (
        <div className="card p-12 flex items-center justify-center text-sm text-slate-400">
          Cargando usuarios...
        </div>
      ) : filtrados.length === 0 ? (
        <div className="card p-12 flex flex-col items-center justify-center text-center gap-3">
          ...
          <p className="text-sm text-slate-500">
            {vista === 'pendientes' ? 'No hay solicitudes pendientes.' : 'No se encontraron usuarios.'}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
          {filtrados.map((u) => (
            <div key={u.id} className={clsx(
              'card p-5 flex flex-col sm:flex-row sm:items-center gap-4 transition-all',
              ...
              <div className="flex items-center gap-4 flex-1 min-w-0">
                <div className={clsx('w-11 h-11 rounded-xl flex items-center justify-center font-display font-bold text-lg shrink-0',
                  u.estado === 'PENDIENTE' ? 'bg-amber-200 text-amber-800' :
                  u.estado === 'ACTIVO' ? 'bg-brand-100 text-brand-700' : 'bg-slate-200 text-slate-500')}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-3 mt-1 flex-wrap">
                    ...
                    <span className="text-xs text-slate-400">
                      Registrado: {new Date(u.created_at).toLocaleDateString('es-BO', { day: '2-digit', month: 'short', year: 'numeric' })}
                    </span>
                  </div>
                </div>
              </div>
              ...
              {u.id !== user?.id && (
                <div className="flex items-center gap-2 shrink-0 flex-wrap">
                  {u.estado === 'PENDIENTE' && (
                    <>
                      <button
                        onClick={() => setConfirmacion({ id: u.id, nombre: u.nombre, tipo: 'aprobar' })}
                      ...
                      <button
                        onClick={() => setConfirmacion({ id: u.id, nombre: u.nombre, tipo: 'bloquear' })}
                        disabled={accionando === u.id}
                  ...
                  {u.estado === 'ACTIVO' && (
                    <>
                      <button
                        onClick={() => setConfirmacion({ id: u.id, nombre: u.nombre, tipo: 'bloquear' })}
                        disabled={accionando === u.id}
                        className="text-xs px-2.5 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 transition-colors font-medium"
                      >
                        Bloquear
                      </button>
                      {u.rol !== 'ADMIN' && (
                        <button
                          onClick={() => setConfirmacion({ id: u.id, nombre: u.nombre, tipo: 'hacer_admin' })}
                          disabled={accionando === u.id}
                          className="text-xs px-2.5 py-1.5 rounded-lg bg-purple-50 text-purple-700 hover:bg-purple-100 border border-purple-200 transition-colors font-medium"
                        >
                          → Admin
                        </button>
                      )}
                  ...
                  {u.estado === 'BLOQUEADO' && (
                    <button
                      onClick={() => setConfirmacion({ id: u.id, nombre: u.nombre, tipo: 'activar' })}
                      disabled={accionando === u.id}
                      className="text-xs px-2.5 py-1.5 rounded-lg bg-green-50 text-green-700 hover:bg-green-100 border border-green-200 transition-colors font-medium"
                    >
                      Reactivar
                    </button>
                  )}
