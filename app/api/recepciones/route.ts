...
// GET /api/recepciones — lista paginada con filtros
...
// POST /api/recepciones — crear recepción + muestras en transacción
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { funcionario_entrega, fecha_roma, fecha_erce, caso_abierto, muestras } = body

    // Validación BDD Feature 2: fecha_erce no puede ser anterior a fecha_roma
    if (new Date(fecha_erce) < new Date(fecha_roma)) {
      return NextResponse.json(
        { ok: false, error: 'La fecha ERCE no puede ser anterior a la fecha ROMA.' },
        { status: 400 }
      )
    }
    ...
    if (!muestras || muestras.length === 0) {
      return NextResponse.json(
        { ok: false, error: 'Debe agregar al menos una muestra.' },
        { status: 400 }
      )
    }
    ...
    const recResult = await sql`
      INSERT INTO recepciones (id_unico, funcionario_entrega, fecha_roma, fecha_erce, caso_abierto, usuario_id)
      VALUES (${recepcionId}, ${funcionario_entrega}, ${fecha_roma}, ${fecha_erce}, ${caso_abierto}, ${session.id})
    ...
    for (const m of muestras as MuestraForm[]) {
      ...
      const mResult = await sql`
        INSERT INTO muestras (
          id_unico, recepcion_id, persona_recolecto, fecha_recoleccion,
          pertenece_a, nombre_muestra, detalle, tipo_muestra_id,
          estudio_pericial_solicitado, codigo_idif_manual, analisis_ia
        ) VALUES (
          ${muestraId}, ${recepcion.id}, ${m.persona_recolecto}, ${m.fecha_recoleccion},
      ...
      // Insertar relaciones tipos de estudio
      if (m.estudio_pericial_solicitado && m.tipos_estudio_ids?.length > 0) {
        for (const teId of m.tipos_estudio_ids) {
          await sql`
            INSERT INTO muestra_tipos_estudio (muestra_id, tipo_estudio_id)
            VALUES (${muestra.id}, ${teId})
