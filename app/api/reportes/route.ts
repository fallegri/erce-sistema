  ...
  try {
      ...
      if (dateFilter && effectiveUserId) {
        rows = await sql`
          SELECT
            ...
            u.ciudad                      AS "Ciudad Registrador",
    ...
    if (tipo === 'por_entrega') {
      const rows = dateFilter
        ? await sql`
            SELECT
              r.funcionario_entrega                                               AS "Funcionario que Entrega",
              COUNT(DISTINCT r.id)::int                                           AS "N Recepciones",
              COUNT(m.id)::int                                                    AS "N Muestras",
              COUNT(CASE WHEN m.estudio_pericial_solicitado THEN 1 END)::int      AS "Con Estudio Pericial",
              COUNT(CASE WHEN r.caso_abierto THEN 1 END)::int                     AS "Casos Abiertos",
    ...
    // ── Por funcionario registrador (usuario ERCE) ────────────────────────────
    if (tipo === 'por_funcionario') {
      const rows = dateFilter
        ? await sql`
            SELECT
              u.nombre                                                            AS "Funcionario Registrador",
              u.ciudad                                                            AS "Ciudad",
              u.email                                                             AS "Email",
              COUNT(DISTINCT r.id)::int                                           AS "N Recepciones",
              COUNT(m.id)::int                                                    AS "N Muestras",
              COUNT(CASE WHEN m.estudio_pericial_solicitado THEN 1 END)::int      AS "Con Estudio Pericial",
              COUNT(CASE WHEN r.caso_abierto THEN 1 END)::int                     AS "Casos Abiertos",
              COUNT(DISTINCT r.funcionario_entrega)::int                          AS "Funcionarios que Entregaron",
              MIN(r.fecha_erce)::text                                             AS "Primera ERCE",
              MAX(r.fecha_erce)::text                                             AS "Ultima ERCE"
            FROM usuarios u
            JOIN recepciones r ON r.usuario_id = u.id
            LEFT JOIN muestras m ON m.recepcion_id = r.id
            WHERE r.fecha_erce BETWEEN ${desde} AND ${hasta}
            GROUP BY u.id, u.nombre, u.ciudad, u.email
            ORDER BY "N Recepciones" DESC`
        : await sql`
            SELECT
              u.nombre                                                            AS "Funcionario Registrador",
              u.ciudad                                                            AS "Ciudad",
