...
// Common columns shared across all reports
const BASE_MUESTRA_COLS = `
  ...
  r.ciudad AS "Ciudad",
  r.caso_abierto AS "Caso Abierto",
  u.nombre AS "Registrado por",
  m.created_at AS "Fecha Registro",
  m.updated_at AS "Última Actualización"
`
...
export async function GET(req: NextRequest) {
  ...
  // ERCE can only see own data
  const effectiveUserId = session.rol === 'ERCE' ? String(session.id) : (userFilter ? usuario_id : null)

  try {
    if (tipo === 'muestras') {
      let rows
      if (dateFilter && effectiveUserId) {
        rows = await sql.unsafe(`SELECT ${BASE_MUESTRA_COLS} ${BASE_JOIN} WHERE r.fecha_erce BETWEEN $1 AND $2 AND r.usuario_id = $3 ORDER BY m.created_at DESC`, [desde, hasta, parseInt(effectiveUserId)])
      } else if (dateFilter) {
        rows = await sql.unsafe(`SELECT ${BASE_MUESTRA_COLS} ${BASE_JOIN} WHERE r.fecha_erce BETWEEN $1 AND $2 ORDER BY m.created_at DESC`, [desde, hasta])
      } else {
        rows = await sql.unsafe(`SELECT ${BASE_MUESTRA_COLS} ${BASE_JOIN} ORDER BY m.created_at DESC LIMIT 500`)
      }
    ...
    if (tipo === 'por_tipo_muestra') {
      const rows = dateFilter
        ? await sql`SELECT tm.nombre AS "Tipo de Muestra", COUNT(m.id)::int AS "Cantidad",
            ...
            array_agg(DISTINCT r.ciudad) FILTER (WHERE r.ciudad IS NOT NULL) AS "Ciudades"
        ...
        : await sql`SELECT tm.nombre AS "Tipo de Muestra", COUNT(m.id)::int AS "Cantidad",
            ...
            JOIN recepciones r ON m.recepcion_id = r.id
            GROUP BY tm.nombre ORDER BY "Cantidad" DESC`
      return NextResponse.json({ ok: true, data: rows.map(r => ({ ...r, Ciudades: Array.isArray(r.Ciudades) ? r.Ciudades.join(', ') : '' })) })
    }

    if (tipo === 'por_tipo_pericia') {
      const rows = dateFilter
        ? await sql`SELECT te.nombre AS "Tipo de Estudio",
            ...
            MAX(r.fecha_erce)::text AS "Última ERCE"
    ...
    if (tipo === 'casos_abiertos') {
      const rows = dateFilter
        ? await sql`SELECT r.id_unico AS "ID Recepción", r.funcionario_entrega AS "Funcionario Entrega",
    ...
    if (tipo === 'sin_pericia') {
      let rows
      if (dateFilter) {
        rows = await sql`SELECT ${sql.unsafe(BASE_MUESTRA_COLS)} ${sql.unsafe(BASE_JOIN)}
            WHERE m.estudio_pericial_solicitado = false AND r.fecha_erce BETWEEN ${desde} AND ${hasta}
            ORDER BY r.fecha_erce DESC`
      } else {
        rows = await sql`SELECT ${sql.unsafe(BASE_MUESTRA_COLS)} ${sql.unsafe(BASE_JOIN)}
            WHERE m.estudio_pericial_solicitado = false ORDER BY r.fecha_erce DESC LIMIT 500`
      }
      return NextResponse.json({ ok: true, data: rows })
    }
    ...
    if (tipo === 'por_entrega') {
      const rows = dateFilter
        ? await sql`SELECT r.funcionario_entrega AS "Funcionario que Entrega",
            ...
            COUNT(CASE WHEN r.caso_abierto THEN 1 END)::int AS "Casos Abiertos",
            array_agg(DISTINCT r.ciudad) FILTER (WHERE r.ciudad IS NOT NULL) AS "Ciudades",
            MIN(r.fecha_erce)::text AS "Primera ERCE", MAX(r.fecha_erce)::text AS "Última ERCE"
