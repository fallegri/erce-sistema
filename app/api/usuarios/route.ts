...
export async function GET() {
  ...
  const usuarios = await sql`
    SELECT id, nombre, ci, email, ciudad, rol, estado, created_at
    FROM usuarios ORDER BY estado ASC, created_at DESC
  `
...
export async function PATCH(req: NextRequest) {
  ...
  if (accion === 'aprobar') {
    await sql`UPDATE usuarios SET estado = 'ACTIVO', rol = 'ERCE' WHERE id = ${id} AND estado = 'PENDIENTE'`
    return NextResponse.json({ ok: true, mensaje: 'Usuario aprobado.' })
  }
  if (accion === 'bloquear') {
    await sql`UPDATE usuarios SET estado = 'BLOQUEADO' WHERE id = ${id} AND id != ${session.id}`
  ...
  if (accion === 'hacer_admin') {
    await sql`UPDATE usuarios SET rol = 'ADMIN' WHERE id = ${id}`
